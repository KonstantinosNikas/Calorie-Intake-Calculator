function calculateCalorieNeeds() {
  const weightPounds = document.getElementById('weight').value; // Weight in pounds
  const activityLevel = document.getElementById('activity').value;
  const bodyFat = document.getElementById('bodyFat').value; // Body fat percentage

  if (weightPounds == "" || activityLevel == "" || bodyFat == "") {
    window.alert("Please fill in all the mandatory fields.");
  } else if (bodyFat > 100 || bodyFat < 0) {
    window.alert("Please enter a valid Body Fat percentage.");
  } else {
    // Convert weight from pounds to kilograms and height from inches to centimeters
    const weight = weightPounds * 0.453592;
...
document.getElementById('unit').addEventListener('click', function() {
  window.location.href = 'file:///Users/kostasnikas/Desktop/Calorie%20Intake%20Calculator/BodyLean/EU/bodylean.html';
});
