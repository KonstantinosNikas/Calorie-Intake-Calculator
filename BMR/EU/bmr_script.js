function calculateCalorieNeeds() {
  ...
  // Rich in Protein carbs 50%, protein 20%, fats 30%
  const calculateMacros = (calories) => {
    const carbs = Math.round(calories * 0.5 / 4); // 4 calories per gram
    const protein = Math.round(calories * 0.2 / 4); // 4 calories per gram
    const fats = Math.round(calories * 0.3 / 9); // 9 calories per gram
...
document.getElementById('unit').addEventListener('click', function() {
    window.location.href = 'file:///Users/kostasnikas/Desktop/Calorie%20Intake%20Calculator/BMR/US/bmr.html';
});
